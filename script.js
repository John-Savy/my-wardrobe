...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Load upload form with name input
  form.innerHTML = `
    <select id="category">
    ...
    <br><br>
    <input type="text" id="custom-name" placeholder="Give this item a name (e.g., 'Blue T-shirt')" style="width: 100%; max-width: 300px;">
    <br><br>
  ...
  async function loadWardrobe() {
    ...
    data.forEach((item) => {
      ...
      const displayName = item.custom_name ? item.custom_name : item.image_url.split('/').pop();
  ...
  window.copyWardrobeToPrompt = async () => {
    ...
    data.forEach(item => {
      const displayName = item.custom_name ? item.custom_name : item.image_url.split('/').pop();
      const fullInfo = `${displayName} (${item.image_url})`;

      switch (item.category) {
        case 'top': 
          categories.tops.push(fullInfo); 
          break;
        case 'bottom': 
          categories.bottoms.push(fullInfo); 
          break;
        case 'shoes': 
          categories.shoes.push(fullInfo); 
          break;
        case 'accessories': 
          categories.accessories.push(fullInfo); 
          break;
      }
